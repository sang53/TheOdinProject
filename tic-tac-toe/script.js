...
DisplayController = function() {
    ...
    function makeCellsSquare() {
        ...
        if (difference >= 0) document.querySelector("body").style.padding = `0px ${difference / 2}px`;
        else {
    ...
    return { makeCellsSquare, placeNames, updateTurn, updateSquare, displayWinner, displayDraw, updateScore, resetBoard, resetScores, toggleButton };
}();
...
EventController = function() {
    ...
    function clickUndo() {
        if (Board.getMoveNumber()) {
            togglePlayerTurn();
            if (!playing) {
                if (Board.checkWin(playerTurn.number))   DisplayController.updateScore(--playerTurn.score, playerTurn.number);
                DisplayController.toggleButton("playAgain");
                playing = true;
            }
            DisplayController.updateSquare(convertIdCoords(...Board.undoMove()), playerTurn.symbol);
            DisplayController.updateTurn(playerTurn.name);
        }
    }

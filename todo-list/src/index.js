...
import { getProjects, saveProjects, removeProjectFromStorage, saveProjectInfo } from "./storageInterface.js";
import { addProjectToPage, setMainDate, removeProjectFromPage, getParentProjectName, removeTaskFromPage, toggleRemoveDefaults, setInputMinDate, addNewTaskInput, addTaskToProject } from "./DOMInterface.js";
...
function clickHandler(event) {
    switch (event.target.className) {
        ...
        case "task-add":
            ...
            break;
        case "task-checkbox":
            toggleTaskComplete(...getTaskObjByElement(event.target, event.target.id.slice(2)));
            break;
        case "dialog-add":
            newProject();
            FORM.reset();
        ...
        case "new-task-button":
            newTask(getProjectByName(getParentProjectName(event.target)), event.target.previousElementSibling.value);
...
function removeProjectFromArray(project) {
    projects.splice(projects.findIndex((element) => element === project), 1);
}
...
function newProject() {
    ...
    FORM.querySelectorAll(".input-task").forEach(function (element) {
        if (element.value)  project.tasks.set(element.value, new Task(element.value));
    });
...
function newTask(project, taskName) {
    ...
    task.taskDiv = addTaskToProject(task, project.mainDiv.querySelector(".main-project"));

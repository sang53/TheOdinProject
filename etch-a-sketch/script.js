...
function adjustGridToSquare() {
  ...
  contentDiv.style.padding =
    difference > 0 ? `0px ${difference}px` : `${-difference}px 0px`;
}
...
function addColor(event) {
  ...
  clicking
    ? changeOpacity(event.target, 0.3)
    : changeOpacity(event.target, 0.1);
}
...
function changeOpacity(element, amount) {
  element.style.opacity = Math.min(
    parseFloat(element.style.opacity) + amount,
    100,
  );
}
...
function buttonPress(event) {
  if (event.target.id === "change-num") {
    ...
    do {
      input = parseInt(
        prompt("Please enter a new resolution from 1 to 100", resolution),
      );
